...
def load_thresholds():
    r = sb.table("system_flags").select("value").eq("key","auto_train_threshold").single().execute().data
...
def start_run(meta):
    return sb.table("training_runs").insert({
        "status":"running","triggered_by": meta.get("triggered_by","auto"),
        "data_count": meta["data_count"], "params_json": meta["params"]
    }).execute().data[0]
...
def main():
    ...
    if args.run_id:
        run = sb.table("training_runs").select("*").eq("id", args.run_id).single().execute().data
        try:
            r = train_and_export({"params": run.get("params_json", {})})
            finish_run(run["id"], "pending_review", metrics=r["metrics"], artifacts=r["artifacts"])
    ...
    meta = {"triggered_by":"auto","data_count": total,"params":{"seed":42,"epochs":80,"imgsz":640,"optimizer":"adamw"}}
    run = start_run(meta)
    try:
